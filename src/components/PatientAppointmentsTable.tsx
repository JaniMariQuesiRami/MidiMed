            ...
            appointments.map((a) => (
              <TableRow key={a.appointmentId}>
                ...
                <TableCell className="flex gap-2">
                  <button onClick={() => onEdit(a)}><Pencil size={16} /></button>
                  <button onClick={() => onDelete(a)}><Trash size={16} /></button>
                  {a.status === 'scheduled' && !a.medicalRecordId && (
                    <button onClick={() => onComplete(a)} className="text-primary hover:underline">Completar</button>
                  )}
                  {a.status === 'completed' && a.medicalRecordId && (
                    ...
                    }} className="text-muted-foreground hover:underline">Ver registro</button>
                  )}
                </TableCell>
              </TableRow>
            ))
